...
//generate a unique number to avoid collisionn in the DB for invoice number
export async function generateUniqueInvoiceNumber(attempts = 8) {
...
export async function getInvoices(req, res){
    try {
        ...
        const invoices = await Invoice.find(q).sort({createAt : -1}).lean();
...
export async function updateInvoice(req,res){
    try {
       const{userId}= getAuth(req) || {};
        ...
        // ensure that it not exists already or not in 
